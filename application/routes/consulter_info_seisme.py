...
@app.route('/ConsulterInfoSeisme/<id>')
def Consulter_info_seisme(id):
    ...
    rapports = list(db.rapportEtudeSeisme.find({'idseisme': id}))
    
    if verifierSession() == "Sismologue" :
        return render_template('consulter_info_seisme.html',poste = "Sismologue",Type_mission=session['type_mission'],donnees=documents, donnees1=donnees1 , donnees2=donnees2 , list_rapports=rapports ,map_html=map_html  )
    if verifierSession() == "Coordonnateur" :
        return render_template('consulter_info_seisme.html',poste = "Coordonnateur",Type_mission=session['type_mission'],donnees=documents, donnees1=donnees1 , donnees2=donnees2 , list_rapports=rapports ,map_html=map_html  )
